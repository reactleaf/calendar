...
const CalendarDatePickerView = forwardRef<CalendarViewportHandle, CalendarDatePickerViewProps>(
  function CalendarDatePickerView(
    {
      mode,
      locale,
      weekStartsOn,
    ...
    const sectionStyleCacheRef = useRef<Map<number, { start: number; style: CSSProperties }>>(new Map())
    const monthInstanceCacheRef = useRef<{
      baseYear: number
      baseMonth: number
      items: (Temporal.PlainYearMonth | undefined)[]
    }>({ baseYear: -1, baseMonth: -1, items: [] })

    const getMonthAt = useCallback(
      (index: number): Temporal.PlainYearMonth => {
        const cache = monthInstanceCacheRef.current
        if (cache.baseYear !== minMonthYear || cache.baseMonth !== minMonthMonth) {
          cache.baseYear = minMonthYear
          cache.baseMonth = minMonthMonth
          cache.items = []
        }
        const cached = cache.items[index]
        if (cached !== undefined) return cached
        const next = plainYearMonthAt(minMonthYear, minMonthMonth, index)
        cache.items[index] = next
        return next
      },
      [minMonthMonth, minMonthYear],
    )

    const getSectionStyle = useCallback((index: number, start: number): CSSProperties => {
      const cache = sectionStyleCacheRef.current
      const cached = cache.get(index)
      if (cached?.start === start) return cached.style
      ...
      cache.set(index, { start, style })
      return style
    }, [])

    useLayoutEffect(() => {
      sectionStyleCacheRef.current.clear()
    }, [minMonthMonth, minMonthYear])

    const virtualMonthsStyle = useMemo<CSSProperties>(
    ...
    return (
      <>
        <CalendarWeekdays />
        <CalendarTodayButton scrollToDate={scrollToDate} scrollRef={scrollRef} placement={todayPlacement} />
        <div
          ref={scrollRef}
        ...
        >
          <div className="calendar__virtualMonths" style={virtualMonthsStyle}>
            {virtualItems.map((vi) => {
              const month = getMonthAt(vi.index)
              ...
              return (
                <section
                  ...
                  data-index={vi.index}
                  style={getSectionStyle(vi.index, vi.start)}
                >
                  <CalendarMonthRow
                    month={month}
                    idPrefix={idPrefix}
